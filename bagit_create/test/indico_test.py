...
def test_parse_indico_metadata():
    ...
    indico_files = [
        ...
        'title': 'The same notes in PDF', 'content_type': 'application/pdf', 
        'metadata': False, 'downloaded': False, 
        ...
        'filename': 'vCHEP2021.slides', 'path': 'vCHEP2021.slides', 
        'title': "the vCHEP slides made with the CERN Slides' app", 
        'content_type': 'application/octet-stream', 'metadata': False, 'downloaded': False, 
        'localpath': 'data/content/vCHEP2021.slides'}
        , {'startDate': {'date': '2021-06-15', 'time': '14:30:00', 'tz': 'Europe/Zurich'}, 
        'endDate': {'date': '2021-06-15', 'time': '16:00:00', 'tz': 'Europe/Zurich'}, 
        'room': '', 'metadata': True, 'downloaded': False, 'localpath': 'data/meta/metadata.json'}]

    # Parsing the metadata to get the list of files
    result_files = indico_pipeline.parse_metadata(os.path.join(os.path.dirname(__file__), "files", "indico_mock_meta.txt"))
...
def test_api_response1():
    ...
    assert metadata_url == "https://indico.cern.ch/export/event/1024767.json?detail=contributions&occ=yes&pretty=yes"
