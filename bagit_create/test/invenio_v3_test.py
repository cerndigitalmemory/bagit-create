...
def test_parse_zenodo_metadata():
    ...
    with open(os.path.join(os.path.dirname(__file__), 'files', 'zenodo_mock_data.txt')) as f:
        data = f.read() 
        data = data.split("###")
...
def test_parse_inveniordm_metadata():
    ...
    # Reading the mock metadata and expected result from file
    with open(os.path.join(os.path.dirname(__file__), 'files', 'inveniordm_mock_data.txt')) as f:
        data = f.read() 
        data = data.split("###")
    ...
    with mock.patch('bagit_create.pipelines.invenio_v3.InvenioV3Pipeline.get_fileslist') as mock_query_get_files:
