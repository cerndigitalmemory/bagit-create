...
def test_api_zenodo_metadata():
    ...
    with open(os.path.join(os.path.dirname(__file__), 'files', 'zenodo_api_data.txt')) as f:
        data = f.read() 
        data = data.split("###")
    ...
    metadata, metadata_url, status_code, metadata_filename = zenodo_pipeline.get_metadata(3974864)
...
def test_api_inveniordm_metadata():
    ...
    # Reading the expected results from file
    with open(os.path.join(os.path.dirname(__file__), 'files', 'inveniordm_api_data.txt')) as f:
        data = f.read() 
        data = data.split("###")
    ...
    # Getting metadata and checking the results
    metadata, metadata_url, status_code, metadata_filename = rdm_pipeline.get_metadata("gjgvm-4mq98")
